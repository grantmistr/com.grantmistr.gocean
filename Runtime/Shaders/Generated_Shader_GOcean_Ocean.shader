    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            SurfaceDescription SurfaceDescriptionFunction(FragInputs input, float3 viewDirection)
            {
                ...
                float lengthMask = _WaterHeight - _WorldSpaceCameraPos_Internal.y;
                lengthMask = sqrt(lengthMask * lengthMask + lengthMaskSquared);
    
            #ifdef HAS_TERRAIN_ON
                float2 uv;
                int slice;
                bool valid;
                GetTerrainSamplingData(preDisplacedPositionXZAbsWS, _TerrainSize, _TerrainPosScaledBounds, float2(0.0, 0.0), _UVMultiplier, _TerrainLookupCoordOffset,
                    _TerrainLookupResolution, _ValidTerrainHeightmapMask, slice, uv, valid);
    
                float terrainHeightmapSample = _TerrainHeightmapArrayTexture.SampleLevel(SamplerState_Linear_Clamp, float3(uv, slice), 0.0).x;
                terrainHeightmapSample = saturate((_WaterHeight - terrainHeightmapSample * _TerrainSize.y) / _WaveDisplacementFade);
                terrainHeightmapSample = valid ? terrainHeightmapSample : 1.0;
    
                float4 terrainShoreWaveSample = _TerrainShoreWaveArrayTexture.SampleLevel(SamplerState_Linear_Clamp, float3(uv, slice), 0.0);
                float2 shoreWaveNormal = (terrainShoreWaveSample.yz * 2.0 - 1.0) * valid;
                shoreWaveNormal = abs(shoreWaveNormal) < float2(0.01, 0.01) ? float2(0.0, 0.0) : shoreWaveNormal;
                float shoreWaveFoam = terrainShoreWaveSample.w * valid * _ShoreWaveFoamAmount;
            #endif            

                float2 uv0 = preDisplacedPositionXZAbsWS / _PatchSize.x;
                float2 uv1 = preDisplacedPositionXZAbsWS / _PatchSize.y;
                ...
                float transitionDistance = saturate(lengthMask / min(min(_ProjectionParams.z, _DisplacementMaxDistance), _PatchSize.x));
                transitionDistance *= transitionDistance;
    
                spectrumSample = lerp(spectrumSample, stochasticSample, transitionDistance);
    
            #ifdef HAS_TERRAIN_ON
                spectrumSample.xyw *= saturate(lengthMaskSquared / (_TerrainSize.x * _TerrainSize.x) + terrainHeightmapSample + 0.2);
                spectrumSample.xy += shoreWaveNormal;
            #endif
                ...
                float foamTextureFadeDistance = saturate(lengthMask / _FoamTextureFadeDistance);
                float2 foamTextureCoord0 = (preDisplacedPositionXZAbsWS - _Time.x * _FoamOffsetSpeed) * _FoamTiling;
                float2 foamTextureCoord1 = (preDisplacedPositionXZAbsWS + _Time.x * _FoamOffsetSpeed) * _SecondaryFoamTiling;
                ...
                float foam = 1.0 - (sceneDepthView - input.positionSS.w) * _EdgeFoamFalloff + _EdgeFoamWidth;
                foam *= IsFarPlane(sceneDepth) ? 0.0 : 1.0;
                foam = saturate(foam) * _EdgeFoamStrength;
            #ifdef HAS_TERRAIN_ON
                foam *= saturate(1.0 - lengthMaskSquared / (_DisplacementMaxDistance * _DisplacementMaxDistance));
                foam = max(foam, shoreWaveFoam);
            #endif    
                foam = saturate(max(foam, spectrumSample.w * 0.25) - foamTextureSample);
                ...
                float smoothness = saturate(1.0 - lengthMask / _SmoothnessTransitionDistance);
            ...
            void ApplyDecalAndGetNormal(FragInputs fragInputs, PositionInputs posInput, SurfaceDescription surfaceDescription, inout SurfaceData surfaceData)
            {
                ...
                normalTS = SurfaceGradientFromPerturbedNormal(fragInputs.tangentToWorld[2], surfaceDescription.NormalWS);
